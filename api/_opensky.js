...
async function llegadasOpenSky(icaoAeropuerto, desde, hasta) {
  const url = "https://opensky-network.org/api/flights/arrival?airport=" +
    encodeURIComponent(icaoAeropuerto) + "&begin=" + desde + "&end=" + hasta;
  ...
  if (!r.ok) {
    const detalle = r.status === 429
      ? "OpenSky ha limitado las consultas por cupo. Registre credenciales en OPENSKY_CLIENT_ID y OPENSKY_CLIENT_SECRET o inténtelo más tarde."
      : "OpenSky respondió " + r.status + ".";
...
/** Solo los dígitos del número de vuelo, sin ceros por delante. */
function digitos(vuelo) {
  const m = String(vuelo || "").toUpperCase().match(/(\d+)/);
  return m ? String(parseInt(m[1], 10)) : "";
}

function prefijoIata(vuelo) {
  const m = String(vuelo || "").toUpperCase().match(/^([A-Z0-9]{2,3})\s*\d/);
  return m ? m[1] : "";
}
...
export async function comprobarVuelo(p) {
  ...
  const num = digitos(p.vuelo);
  if (!num) {
    return { encontrado: false, error: "Hace falta el número de vuelo.", avisos };
  }
  const prefijo = prefijoIata(p.vuelo);
  const icaoCompania = COMPANIAS[prefijo] || "";
  if (!icaoCompania && prefijo) {
    avisos.push("El prefijo " + prefijo + " no está en la tabla de compañías: la búsqueda se hace solo por número de vuelo.");
  }
  ...
  let llegadas;
  try {
    llegadas = await llegadasOpenSky(destino.icao, desde, hasta);
  } catch (e) {
    return { encontrado: false, error: e.message, avisos };
  }

  const candidatos = llegadas.filter((f) => {
    const ind = String(f.callsign || "").trim().toUpperCase();
    if (!ind) return false;
    if (digitos(ind) !== num) return false;
    if (icaoCompania) return ind.startsWith(icaoCompania);
    return true;
  });

  if (candidatos.length === 0) {
    avisos.push(
      "OpenSky no tiene registrada esa llegada. Puede deberse a falta de cobertura ADS-B, " +
      "a que la compañía use un indicativo distinto del número comercial o a que el vuelo " +
      "quede fuera del histórico disponible. Compruébelo a mano con el panel del aeropuerto " +
      "o el certificado de la aerolínea."
    );
    return { encontrado: false, consultadas: llegadas.length, avisos };
  }
