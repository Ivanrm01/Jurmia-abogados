...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (accion === "actualizar") {
      ...
      ["numero", "aerolinea", "fecha", "origen", "destino", "localizador", "horaLlegadaPrevista"].forEach((k) => {
    ...
    if (accion === "comprobar") {
      ...
      const resultado = await comprobarVuelo({
      ...
      e.historial = (e.historial || []).concat([{
        ...
        que: resultado.encontrado
          ? "Comprobación ADS-B: aterrizaje " + resultado.aterrizajeUtc
          : "Comprobación ADS-B sin resultado"
      }]).slice(-40);
